...
# TODO: Create Response model or align frontend expectations to be TransactionRead[]
@router.get("/")
async def get_transactions(
    user: UserRead = Depends(get_current_user),
    service: TransactionService = Depends(get_transaction_service),
) -> dict[str, list[TransactionRead]]:
...
def delete_transaction(
    transaction_id: str, user: UserRead = Depends(get_current_user)
) -> None:
